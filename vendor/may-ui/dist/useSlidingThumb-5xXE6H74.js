import { jsx as Z, jsxs as ma, Fragment as Ea } from "react/jsx-runtime";
import { useId as Ca, useState as ca, useRef as w, useEffect as j, useCallback as W, useSyncExternalStore as ba, isValidElement as qa, cloneElement as Ia, useLayoutEffect as ha, useContext as ta, createContext as Ba, useMemo as Pa, createElement as sa, Fragment as Fa } from "react";
function ya(...a) {
...
function Ya(a, r, t, i) {
  const l = document.documentElement.clientWidth, f = document.documentElement.clientHeight, [d, v] = t.split("-"), k = v ?? "center", C = {
    top: a.top - U,
    bottom: f - a.bottom - U,
    left: a.left - U,
    right: l - a.right - U
  }, O = (d === "top" || d === "bottom" ? r.height : r.width) + i, I = $a[d], p = C[d] >= O || C[I] <= C[d] ? d : I, E = p === "top" || p === "bottom", m = E ? C[p] - i : f - U * 2, A = Math.min(r.height, m), g = Math.min(r.width, l - U * 2);
  let T = 0, b = 0;
  p === "top" ? b = a.top - i - A : p === "bottom" ? b = a.bottom + i : p === "left" ? T = a.left - i - g : T = a.right + i;
  const z = E ? a.left : a.top, L = E ? a.width : a.height, P = E ? g : A;
  let F = z + (L - P) / 2;
  k === "start" ? F = z : k === "end" && (F = z + L - P);
  const G = (E ? l : f) - U - P;
  F = Math.max(U, Math.min(F, Math.max(U, G))), E ? T = F : b = F;
...
function de({ sheets: a = [] }) {
  const r = ie();
  return /* @__PURE__ */ Z(Ea, { children: [ka, ...a].map((t) => /* @__PURE__ */ Z(
    "style",
    {
      href: t.href,
...
function ge() {
  ...
  ua = 1, Object.defineProperty(u, "__esModule", {
    value: !0
  }), u.linear = a, u.easeInSine = r, u.easeOutSine = t, u.easeInOutSine = i, u.easeInQuad = l, u.easeOutQuad = f, u.easeInOutQuad = d, u.easeInCubic = v, u.easeOutCubic = k, u.easeInOutCubic = C, u.easeInQuart = q, u.easeOutQuart = O, u.easeInOutQuart = I, u.easeInQuint = p, u.easeOutQuint = E, u.easeInOutQuint = m, u.easeInExpo = A, u.easeOutExpo = g, u.easeInOutExpo = T, u.easeInCirc = b, u.easeOutCirc = z, u.easeInOutCirc = L, u.easeInBack = P, u.easeOutBack = F, u.easeInOutBack = G, u.easeInElastic = R, u.easeOutElastic = V, u.easeInOutElastic = N, u.easeOutBounce = D, u.easeInBounce = H, u.easeInOutBounce = M;
  function a(e) {
    return e;
  }
  function r(e) {
...
function Fe(a, r) {
  ...
  let v = !1, k = t === "both" ? null : t, C = 0, q = 0, O = [];
  const I = (m) => {
    m.isPrimary && (v = !0, k = t === "both" ? null : t, C = m.clientX, q = m.clientY, O = [{ x: m.clientX, y: m.clientY, t: m.timeStamp }], a.setPointerCapture(m.pointerId), l == null || l());
  }, p = (m) => {
    if (!v) return;
    const A = m.clientX - C, g = m.clientY - q;
  ...
  return a.addEventListener("pointerdown", I), a.addEventListener("pointermove", p), a.addEventListener("pointerup", E), a.addEventListener("pointercancel", E), () => {
    a.removeEventListener("pointerdown", I), a.removeEventListener("pointermove", p), a.removeEventListener("pointerup", E), a.removeEventListener("pointercancel", E);
  };
}
...
function xe(a, { x: r, width: t }, {
  ...
  const I = f ? O : 1, p = f ? 1 + (O - 1) * 1.1 : 1, m = k ? "transform 0s" : l ? `transform ${d ? 1 : he}ms linear` : `transform ${d ? 1 : q}ms ${C}`, g = r - t * (I - 1) / 2 + v, T = i === "block" ? `translateY(${g}px) scaleY(${t * I}) scaleX(${p})` : `translateX(${g}px) scaleX(${t * I}) scaleY(${p})`;
  let b = fa.get(a);
...
function We({
  ...
  const O = xa(), I = i !== void 0, p = w(null), E = w(null), m = w([]), A = w(!0), g = w(0), T = w(null), b = w(!1), z = w(null), L = w(0), P = w(NaN), F = w(""), G = w(-1), R = w(t), V = w(t);
  V.current = t;
    ...
    (c, x = {}) => {
      ...
      if (T.current !== null) {
        ...
        if (Y && $) {
          ...
          g.current = ea < na ? -ra(na - ea, C) : ea > ia ? ra(ea - ia, C) : 0;
  ...
  return { trackRef: p, thumbRef: E, registerItem: Ma, onPointerDown: S };
}
export {
  xa as A,
  Ra as B,
...
//# sourceMappingURL=useSlidingThumb-5xXE6H74.js.map
